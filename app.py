...
from autorater_llm import evaluate_contributor_with_groq
...
if analyze:
    ...
    with left:
            ...
            with st.spinner("Fetching commit file lists (this may take time)..."):
                for c in sample_commits:
                    ...
                    try:
                        ...
                        # fallback to raw commit author info
                        author = (c.get("author") or {}).get("login") or (c.get("commit") or {}).get("author",{}).get("name","unknown")
                        files = []
                    for f in files:
                        path = f.get("filename","")
        ...
        if file_rows:
            ...
            # pick top authors and top modules
    ...
    for login in sample:
        ...
        # call autorater Groq
        result = evaluate_contributor_with_groq(prs)
        st.json(result)
        time.sleep(0.25)
